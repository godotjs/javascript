...
private:
	static QuickJSBuiltinBinder *singleton;
	JSContext *ctx;
...
public:
	_FORCE_INLINE_ static QuickJSBuiltinBinder *get_singleton() { return singleton; }

	QuickJSBuiltinBinder();
	~QuickJSBuiltinBinder();

	void initialize(JSContext *p_context);
	void uninitialize();

	_FORCE_INLINE_ void set_classid(Variant::Type p_type, JSClassID p_id) {
		builtin_class_map[p_type].id = p_id;
	}

	_FORCE_INLINE_ JSClassID get_classid(Variant::Type p_type) {
		return builtin_class_map[p_type].id;
	}

	_FORCE_INLINE_ BuiltinClass &get_class(Variant::Type p_type) {
		return *(builtin_class_map + p_type);
	}

	JSValue new_object_from(const Variant &p_val);
};
