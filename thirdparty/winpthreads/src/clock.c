...
static GetSystemTimeAsFileTime_t try_load_GetSystemPreciseTimeAsFileTime()
{
    ...
    _InterlockedExchangePointer((PVOID volatile *)&GetSystemTimeAsFileTime_p, get_time);
    return get_time;
}
...
static WINPTHREADS_INLINE GetSystemTimeAsFileTime_t load_GetSystemTimeBestAsFileTime() {
    ...
    atomic_init(&GetSystemTimeAsFileTime_p, NULL);
    GetSystemTimeAsFileTime_t get_time =
        atomic_load_explicit(&GetSystemTimeAsFileTime_p, memory_order_relaxed);
    if (get_time == NULL) {
        get_time = try_load_GetSystemPreciseTimeAsFileTime();
        atomic_store_explicit(&GetSystemTimeAsFileTime_p, get_time, memory_order_relaxed);
    }
    return get_time;
}
